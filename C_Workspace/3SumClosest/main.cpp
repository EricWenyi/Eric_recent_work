#include <iostream>
#include<bits/stdc++.h>

using namespace std;

class Solution {
public:


    int threeSumClosest(vector<int>& nums, int target) {
        std::sort(nums.begin(),nums.end());

        int i = 0;
        int interval = INT_MAX;
        int closest = 0;

        for(i = 0; i  < nums.size(); i++){
            int front = i + 1, end = nums.size() - 1;


            while(front < end){

                int temp = abs(nums[front] + nums[end] + nums[i] - target);

                if(temp < interval){
                    interval = temp;
                    closest = nums[i]  + nums[front] + nums[end];
                }

                if(closest == target)
                    return closest;


                //while(front < end && nums[front] == nums[front + 1]) front++;  it is tricky that you have to consider the condition that the second and the third are the same

                //while(front < end && nums[end] == nums[end - 1]) end--;

                if(nums[i]  + nums[front] + nums[end] > target){
                    end--;
                }
                else if(nums[i]  + nums[front] + nums[end] < target){
                    front++;
                }

            }

            while(i < nums.size() && nums[i] == nums[i + 1]) i++;
        }

        return closest;
    }
};


int main()
{
    int arr[] = {13,2,0,-14,-20,19,8,-5,-13,-3,20,15,20,5,13,14,-17,-7,12,-6,0,20,-19,-1,-15,-2,8,-2,-9,13,0,-3,-18,-9,-9,-19,17,-14,-19,-4,-16,2,0,9,5,-7,-4,20,18,9,0,12,-1,10,-17,-11,16,-13,-14,-3,0,2,-18,2,8,20,-15,3,-13,-12,-2,-19,11,11,-10,1,1,-10,-2,12,0,17,-19,-7,8,-19,-17,5,-5,-10,8,0,-12,4,19,2,0,12,14,-9,15,7,0,-16,-5,16,-12,0,2,-16,14,18,12,13,5,0,5,6};
    vector<int> nums(arr,arr + sizeof(arr) / sizeof(int));
    int target = -59;


    Solution s;
    cout<<s.threeSumClosest(nums,target);


    return 0;
}
